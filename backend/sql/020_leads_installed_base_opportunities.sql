...
CREATE SEQUENCE IF NOT EXISTS public.sales_leads_number_seq
...
CREATE TABLE IF NOT EXISTS sales_leads (
  lead_id text PRIMARY KEY DEFAULT public.mii_next_record_id(
    'LEAD', 'public.sales_leads_number_seq'::regclass
  ),
  ...
  CONSTRAINT fk_sales_leads_owner FOREIGN KEY (owner_id)
    REFERENCES app_users(user_id) ON DELETE RESTRICT,
  CONSTRAINT fk_sales_leads_customer FOREIGN KEY (converted_customer_id)
    REFERENCES customers(customer_id) ON DELETE SET NULL,
  CONSTRAINT fk_sales_leads_created_by FOREIGN KEY (created_by)
    REFERENCES app_users(user_id) ON DELETE SET NULL,
  CONSTRAINT fk_sales_leads_updated_by FOREIGN KEY (updated_by)
    REFERENCES app_users(user_id) ON DELETE SET NULL,
  CONSTRAINT chk_sales_leads_account_type CHECK (
    account_type IN ('organization','individual')
  ),
  CONSTRAINT chk_sales_leads_status CHECK (
    status IN ('new','contacted','qualified','converted','disqualified')
  ),
  CONSTRAINT chk_sales_leads_name CHECK (length(btrim(name)) > 0),
  CONSTRAINT chk_sales_leads_contact CHECK (length(btrim(contact_name)) > 0),
  CONSTRAINT chk_sales_leads_phone CHECK (length(btrim(contact_phone)) > 0),
  CONSTRAINT chk_sales_leads_value CHECK (estimated_value >= 0),
  CONSTRAINT chk_sales_leads_next_action CHECK (
    (next_action IS NULL AND next_action_at IS NULL)
    OR (next_action IS NOT NULL AND next_action_at IS NOT NULL)
  ),
  CONSTRAINT chk_sales_leads_disqualified CHECK (
    status <> 'disqualified'
    OR length(btrim(COALESCE(disqualified_reason, ''))) > 0
  ),
  CONSTRAINT chk_sales_leads_converted CHECK (
    status <> 'converted'
    OR (converted_customer_id IS NOT NULL AND converted_at IS NOT NULL)
  )
);

ALTER SEQUENCE public.sales_leads_number_seq OWNED BY sales_leads.lead_id;

CREATE INDEX IF NOT EXISTS idx_sales_leads_owner_status_action
  ON sales_leads (owner_id, status, next_action_at);
CREATE INDEX IF NOT EXISTS idx_sales_leads_name
  ON sales_leads (lower(name));
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_sales_leads_opportunity') THEN
    ALTER TABLE sales_leads ADD CONSTRAINT fk_sales_leads_opportunity
