...
(async () => {
  try {
    const checks = await pool.query(`
      SELECT
        to_regclass('public.sales_leads') IS NOT NULL AS leads_table,
        ...
        NOT EXISTS (SELECT 1 FROM sales_leads l LEFT JOIN app_users u ON u.user_id=l.owner_id WHERE u.user_id IS NULL) AS no_orphan_lead_owners,
        NOT EXISTS (
