            ...
            <TableBody>
              {filtered.map((item) => (
                <TableRow key={item.opportunity_id} hover>
                  <TableCell><Typography variant="body2" fontWeight={700} sx={truncateTextSx}>{item.title}</Typography><Typography variant="caption" color="text.secondary" sx={truncateTextSx}>{item.customer_name} · {item.opportunity_id}{item.product_interest ? ` · ${item.product_interest}` : ""}</Typography></TableCell>
